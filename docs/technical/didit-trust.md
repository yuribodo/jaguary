# Didit and customer trust

| Metadata | Value |
| --- | --- |
| Status | Real adapter implemented; optional by mode |
| Purpose | Add customer identity evidence and biometric consent evidence |
...
Didit verifies the authenticated customer through a hosted identity workflow. Bound normalizes a newly approved result into the `PRINCIPAL_IDENTITY` assurance claim. Didit does not certify TravelBot's code, key, build fingerprint, or complete agent identity, and it never returns a Bound `ALLOW`.
...
| `EXTERNAL_REQUIRED` | A current customer-bound `VERIFIED` attestation is mandatory at the authority/biometric seam. Unknown or unavailable fails closed. |

## Customer-attestation flow

1. An authenticated principal explicitly consents and starts an attestation with idempotency and CSRF protection.
2. Bound verifies access to the agent. Owner-scoped agents require ownership; public TravelBot accepts every authenticated customer and binds the session to that customer.
...
5. The verified result is bound to the customer principal, agent, mandate, that customer's onboarding assessment, and immutable mandate-terms hash.
