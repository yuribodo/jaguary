...
export const purchaseDisputes = pgTable("purchase_disputes", {
  disputeId: varchar("dispute_id", { length: 128 }).primaryKey(),
  receiptId: varchar("receipt_id", { length: 128 }).notNull().references(() => orders.receiptId),
  orderId: varchar("order_id", { length: 128 }).notNull().references(() => orders.orderId),
  authorizationId: varchar("authorization_id", { length: 128 }).notNull().references(() => authorizations.authorizationId),
  paymentId: varchar("payment_id", { length: 128 }).notNull().references(() => payments.paymentId),
  principalId: varchar("principal_id", { length: 128 }).notNull().references(() => principals.principalId),
  merchantId: varchar("merchant_id", { length: 128 }).notNull(),
  ...
  liableParty: varchar("liable_party", { length: 24 }).notNull(),
  financialOutcome: varchar("financial_outcome", { length: 32 }).notNull(),
  resolutionCode: varchar("resolution_code", { length: 64 }).notNull(),
  evidence: jsonb("evidence").$type<PurchaseDisputeEvidence>().notNull(),
  evidenceHash: char("evidence_hash", { length: 64 }).notNull(),
  correlationId: varchar("correlation_id", { length: 128 }).notNull(),
  idempotencyKey: varchar("idempotency_key", { length: 128 }).notNull(),
  ...
  unique("purchase_disputes_receipt_unique").on(table.receiptId),
  unique("purchase_disputes_idempotency_unique").on(table.idempotencyKey),
  check("purchase_disputes_id_check", identifierCheck(table.disputeId)),
  check("purchase_disputes_reason_check", sql`${table.reason} IN (${sqlList(purchaseDisputeReasonSchema.options)})`),
  check("purchase_disputes_status_check", sql`${table.status} IN (${sqlList(purchaseDisputeStatusSchema.options)})`),
  check("purchase_disputes_verdict_check", sql`${table.verdict} IN (${sqlList(purchaseDisputeVerdictSchema.options)})`),
  check("purchase_disputes_liable_party_check", sql`${table.liableParty} IN (${sqlList(purchaseDisputeLiablePartySchema.options)})`),
  check("purchase_disputes_financial_outcome_check", sql`${table.financialOutcome} IN (${sqlList(purchaseDisputeFinancialOutcomeSchema.options)})`),
  check("purchase_disputes_resolution_code_check", sql`${table.resolutionCode} IN (${sqlList(purchaseDisputeResolutionCodeSchema.options)})`),
  check("purchase_disputes_evidence_check", sql`jsonb_typeof(${table.evidence}) = 'object'`),
  check("purchase_disputes_evidence_hash_check", hashCheck(table.evidenceHash)),
  check("purchase_disputes_correlation_id_check", identifierCheck(table.correlationId)),
  check("purchase_disputes_idempotency_check", sql`length(${table.idempotencyKey}) BETWEEN 8 AND 128 AND ${identifierCheck(table.idempotencyKey)}`),
  check("purchase_disputes_resolution_time_check", sql`${table.openedAt} <= ${table.resolvedAt}`),
  index("purchase_disputes_principal_opened_idx").on(table.principalId, table.openedAt),
  index("purchase_disputes_merchant_opened_idx").on(table.merchantId, table.openedAt),
]);
