...
For the recommended local setup, run this from the repository root:
...
The setup command starts the development PostgreSQL container, creates
`backend/.env` from the example when absent, applies migrations, generates the
local TravelBot P-256 signing key and AES approval-state key, and seeds the
matching public identity and logical demo credential. It does not overwrite a
complete existing key set and fails when only part of a key set is configured.

When running the backend package directly instead, copy `backend/.env.example`
to `backend/.env`. `DATABASE_URL` is required and must be a `postgres://` or
`postgresql://` URL with a host and database name.
...
### Generated locally versus supplied manually

`pnpm setup:demo` generates and persists these development-only values in the
...
The private JWK never enters PostgreSQL. Only its public P-256 coordinates,
key ID, and build fingerprint are registered for `agent_travelbot`. The command
also creates the logical credential template used by the fake local payment
executor. Generated local material must not be reused in production.

These provider credentials cannot be generated by Jaguary and must be added to
`backend/.env` manually to enable the live conversational flight path:
...
Didit, Google OIDC, Langfuse, and Yuno variables are optional locally. If they
are enabled, their complete provider-issued configuration is required by the
...
```bash
pnpm dev:backend
pnpm --filter @bound/backend db:setup:demo
